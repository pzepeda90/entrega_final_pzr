...
import CartNotificationService from '../../services/CartNotificationService';
...
const Cart = () => {
  ...
  const handleUpdateQuantity = (id, quantity, name) => {
    ...
    CartNotificationService.itemUpdated({ name, quantity });
  };

  const handleRemoveItem = (id, name) => {
    dispatch(removeFromCart(id));
    CartNotificationService.itemRemoved({ name });
  };
  ...
  const handleClearCart = async () => {
    ...
    if (result.isConfirmed) {
      dispatch(clearCart());
      CartNotificationService.cartCleared();
    }
  };
                  ...
                  <div className="flex-1 flex items-end justify-between text-sm">
                    <div className="flex items-center">
                      ...
                      <input
                        ...
                        value={item.quantity}
                        min="1"
                        onChange={(e) => handleUpdateQuantity(item.id, parseInt(e.target.value), item.name)}
                      />
                      <button
